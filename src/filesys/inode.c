  ...
  if (disk_inode != NULL)
    {
      ...
      if (free_map_allocate (sectors, &disk_inode->start)) 
        {
          memcpy (buffer_cache_get_buffer (), disk_inode, BLOCK_SECTOR_SIZE);
          buffer_cache_write (sector);
          if (sectors > 0) 
            {
              size_t i;
              
              memset (buffer_cache_get_buffer (), 0, BLOCK_SECTOR_SIZE);
              for (i = 0; i < sectors; i++)
...
inode_open (block_sector_t sector)
{
  ...
  memcpy (&inode->data, buffer_cache_get_buffer (), BLOCK_SECTOR_SIZE);
  lock_release (buffer_cache_get_lock ());
  return inode;
}
  ...
  while (size > 0) 
    {
      ...
      memcpy (buffer + bytes_read, buffer_cache_get_buffer () + sector_ofs, chunk_size);
  ...
  while (size > 0) 
    {
      ...
      if (sector_ofs > 0 || chunk_size < sector_left)
        ...
        memset (buffer_cache_get_buffer (), 0, BLOCK_SECTOR_SIZE);
      memcpy (buffer_cache_get_buffer () + sector_ofs, buffer + bytes_written, chunk_size);
