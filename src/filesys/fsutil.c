...
fsutil_rm (char **argv) 
{
  ...
  printf ("Deleting '%s'...\n", file_name);
  if (!filesys_remove_at_root (file_name))
  ...
  for (;;)
    {
      ...
      if (type == USTAR_EOF)
      ...
      else if (type == USTAR_REGULAR)
        {
          ...
          if (!filesys_create_file_at_root (file_name, size))
            PANIC ("%s: create failed", file_name);
          dst = filesys_open_file_at_root (file_name);
          if (dst == NULL)
            PANIC ("%s: open failed", file_name);
...
fsutil_append (char **argv)
{
  ...
  /* Open source file. */
  src = filesys_open_file_at_root (file_name);
  if (src == NULL)
    PANIC ("%s: open failed", file_name);
